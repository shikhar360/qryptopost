  ...
  return (
    <div className="pt-24 bg-[#240046] min-h-screen overflow-hidden ">
      ...
      <div className="grid grid-cols-4 max-h-[90%] w-[80%] mx-auto overflow-y-scroll gap-y-5 scrollbar-hide">
       { mailers.map(( mailer : IMailer, idx : number) =><div key={idx} className={`col-span-4 md:col-span-1 min-h-[10rem]  w-[90%] mx-auto rounded-xl py-3 px-2 ${
          ...
          <div
           ...
           <span >{mailer.name ? mailer.name.toUpperCase() : mailer.ethAddress}</span>
           <span className={`w-full truncate text-center mt-2`}>{count} Subscriber</span>
           <button disabled={(isLoading  || allSubs.some((val : any) => val.sub_id == mailer.id))} onClick={()=>subscribeTo(mailer.id)}  className={`mt-8 py-1 px-3  rounded-xl ${allSubs.some((val : any) => val.sub_id == mailer.id) ? "bg-lime-400 text-black" : "bg-violet-500" }`} >{isLoading ? <Loading w={"w-10"} simple={true}/> : "Subscribed" }</button>
          </div>
        </div>)}
      </div>
      <ToastContainer
