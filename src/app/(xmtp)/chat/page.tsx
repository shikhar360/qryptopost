    ...
    <XMTPProvider>
      ...
      <div className="flex min-h-screen pt-28 px-10 bg-[#240046] overflow-hidden">
        <div className="w-1/4 bg-[#17002B] min-h-full rounded-xl p-4">
          <h2 className="text-white flex w-full justify-between items-center mb-4">
            Chat Contacts{" "}
          </h2>
          <div>
            {chatUser &&
              chatUser?.map((user: any, idx: number) => (
                <div
                  key={idx}
                  className="text-white truncate w-full cursor-pointer mb-2"
                >
                  {user.client.knownPublicKeyBundles[0]}Chat {idx + 1}
                </div>
              ))}
          </div>
        </div>
        <div className="w-3/4 p-4">
          {selectedContact ? (
            <div className={`bg-white/5 p-2`}>
              ...
              <div className="mt-4 flex">
                ...
                <button
                  ...
                  className="ml-2 bg-violet-500 text-white p-2 rounded"
                >
                  Send
                </button>
              </div>
            ...
            <div className="text-white flex flex-col  bg-white/5 w-full min-h-full  rounded-xl p-4 ">
              ...
              />
              <span
                className={`${debouncedValue ? "block" : " hidden "} my-2 ${
                  canMess ? "text-lime-400" : " text-rose-500 "
                }`}
              >
                {canMess
                  ? "You can message this Address"
                  : "This user didnt registered on XMTP"}
              </span>

              <div
                onClick={() => setSelectedContact(debouncedValue)}
                className={` ${
                  canMess ? "block" : " hidden "
                } bg-violet-500 py-2 px-4 rounded-xl cursor-pointer w-[10rem] mt-4 `}
              >
                Send Chat
              </div>
              <br />
